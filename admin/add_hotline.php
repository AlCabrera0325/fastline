...
$message = '';
$error   = '';
$editData = null; // holds hotline being edited
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    if (empty($name) || empty($category) || empty($phone)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO hotlines (name, category, phone, description, city, barangay, is_active)
                               VALUES (?, ?, ?, ?, ?, ?, 1)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
    ...
    if (empty($name) || empty($category) || empty($phone)) {
        $error = 'Name, category, and phone are required.';
        // Reload edit form with current data
        $stmt = $pdo->prepare("SELECT * FROM hotlines WHERE id = ?");
        $stmt->execute([$id]);
        $editData = $stmt->fetch(PDO::FETCH_ASSOC);
    } else {
        $stmt = $pdo->prepare("UPDATE hotlines SET name=?, category=?, phone=?, description=?, city=?, barangay=? WHERE id=?");
        $stmt->execute([$name, $category, $phone, $description, $city, $barangay ?: 'all', $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'toggle') {
    $id = (int)($_POST['id'] ?? 0);
    $stmt = $pdo->prepare("UPDATE hotlines SET is_active = NOT is_active WHERE id = ?");
    $stmt->execute([$id]);
...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --green:rgba(40,167,69,0.8); --yellow:rgba(255,193,7,0.8);
        }
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-logout { background:transparent; border:1px solid var(--border); color:var(--muted); padding:8px 16px; border-radius:8px; cursor:pointer; font-size:0.85rem; text-decoration:none; display:flex; align-items:center; gap:6px; transition:all 0.2s; }
        .btn-logout:hover { border-color:var(--red); color:#ff8080; }

        /* ── Tabs ── */
        .page-tabs { display:flex; gap:0; border-bottom:1px solid var(--border); padding:0 32px; background:var(--surface); }
        .page-tab { padding:14px 24px; cursor:pointer; font-size:0.88rem; font-weight:600; color:var(--muted); border:none; background:none; border-bottom:2px solid transparent; transition:all 0.2s; display:flex; align-items:center; gap:8px; }
        .page-tab:hover { color:var(--text); }
        .page-tab.active { color:var(--red); border-bottom-color:var(--red); }
        .page-tab .count { background:rgba(201,42,42,0.15); color:#ff8080; font-size:0.7rem; padding:2px 7px; border-radius:10px; }
        ...
        .field input:focus, .field select:focus, .field textarea:focus {
            border-color:var(--red); box-shadow:0 0 0 3px var(--red-glow); background:rgba(201,42,42,0.04);
        }
        .field select option { background:#1e1e1e; }
        .field textarea { resize:vertical; min-height:72px; }

        .btn-submit {
            width:100%; padding:12px; background:var(--red); color:white; border:none;
            border-radius:8px; font-family:'DM Sans',sans-serif; font-size:0.95rem; font-weight:600;
        ...
        .btn-cancel {
            ...
            transition:all 0.2s; margin-top:8px; text-decoration:none;
        }
        ...
        .alert-edit    { background:rgba(255,193,7,0.1);  border:1px solid rgba(255,193,7,0.3);  color:#ffd43b; }
        ...
        .btn-edit {
            background:transparent; border:1px solid rgba(255,193,7,0.35); color:#ffd43b;
        ...
        .btn-toggle {
            background:transparent; border:1px solid rgba(255,255,255,0.12); color:var(--muted);
            padding:5px 10px; border-radius:6px; cursor:pointer; font-size:0.78rem;
        ...
        .btn-delete {
            background:transparent; border:1px solid rgba(201,42,42,0.3); color:#ff8080;
            padding:5px 10px; border-radius:6px; cursor:pointer; font-size:0.78rem;
            display:inline-flex; align-items:center; gap:4px; transition:all 0.2s;
        }
        ...
        .editing-banner {
            ...
            font-size:0.85rem; color:#ffd43b; display:flex; align-items:center; gap:8px;
        }
    </style>
</head>
<body>
...
<nav class="navbar">
    ...
    <div class="nav-right">
        ...
        <a href="../index.php" class="btn-logout"><i class="fas fa-home"></i> View Site</a>
...
<!-- Page Tabs -->
<div class="page-tabs">
    <button class="page-tab active" onclick="showPage('hotlines', this)">
        <i class="fas fa-phone-alt"></i> Hotlines
        <span class="count"><?php echo count($hotlines); ?></span>
    </button>
    <button class="page-tab" onclick="showPage('users', this)">
...
<div id="page-hotlines" class="page active">
    <div style="max-width:1200px; margin:0 auto; padding:32px 24px;">
        ...
        <div class="stats">
            <?php
            $active   = count(array_filter($hotlines, fn($h) => $h['is_active'] ?? 1));
            $inactive = count($hotlines) - $active;
            $cats = ['police'=>0,'medical'=>0,'fire'=>0,'disaster'=>0];
            foreach ($hotlines as $h) if (isset($cats[$h['category']])) $cats[$h['category']]++;
            ?>
            <div class="stat-card"><div class="num"><?php echo count($hotlines); ?></div><div class="lbl">Total Hotlines</div></div>
            <div class="stat-card"><div class="num"><?php echo $active; ?></div><div class="lbl">Active</div></div>
            <div class="stat-card"><div class="num"><?php echo $inactive; ?></div><div class="lbl">Inactive</div></div>
                ...
                <form method="POST" action="">
                    ...
                    <div class="field">
                        ...
                        <input type="text" name="name" required
                               placeholder="e.g. Angeles City Police"
                                ...
                                <td>
                                    <div class="actions">
                                        ...
                                        <form method="POST" action="" style="display:inline">
                                            ...
                                            <button type="submit" class="btn-toggle" title="<?php echo ($h['is_active'] ?? 1) ? 'Deactivate' : 'Activate'; ?>">
...
// Auto-open users tab if ?tab=users in URL
if (new URLSearchParams(window.location.search).get('tab') === 'users') {
    showPage('users', document.querySelectorAll('.page-tab')[1]);
}
</script>
